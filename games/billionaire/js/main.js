...
function safeInitialize(name, initializer) {
  try {
    ...
    console.error(`${name} 초기화 오류:`, error);
  }
}
...
function registerBaseEvents() {
  if (elements.startBtn) {
    ...
    console.error("startBtn 요소를 찾을 수 없습니다.");
  }
...
function initializeGameModules() {
  safeInitialize("메인 메뉴", initializeMainMenu);
  safeInitialize("강화", initializeUpgrade);
  safeInitialize("직업", initializeJob);
  safeInitialize("도박", initializeGambling);
  safeInitialize("사업", initializeBuilding);
  safeInitialize("알바", initializeEmployee);
  safeInitialize("펫", initializePet);
}
